    ...
    IF @IsBalanceBothSide = 1
        BEGIN
            ...
            FROM (
                     ...
                     WHERE GL.PostedDate <= @ToDate AND GL.CompanyID IN (SELECT id FROM Func_getCompany (@CompanyID, @IncludeDependentBranch))
            ...
            FROM (
                     SELECT A.AccountNumber,
                            ...
                            CASE
                                WHEN (A.DetailType LIKE '%0%' OR A.DetailType LIKE '%1%' OR A.DetailType LIKE '%2%') THEN GL.AccountObjectID
                                ELSE NULL
                     ...
                     WHERE GL.AccountNumber NOT LIKE '0%' AND A.CompanyID IN (SELECT id FROM Func_getCompany (@CompanyID, @IncludeDependentBranch))
            ...
            WHERE A.Grade <= @MaxAccountGrade AND A.CompanyID IN (SELECT id FROM Func_getCompany (@CompanyID, @IncludeDependentBranch))
            GROUP BY A.ID,
                     A.AccountNumber,
                     CASE
                         WHEN D.CurrencyID <> '' THEN D.CurrencyID

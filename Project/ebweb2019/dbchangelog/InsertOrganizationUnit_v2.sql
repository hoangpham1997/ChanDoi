...
-- =============================================
ALTER proc [dbo].[InsertOrganizationUnit] @companyID UNIQUEIDENTIFIER, @mainCurrency NVARCHAR(3)
as
begin
    -- ADD DATA ACCOUNT LIST
    INSERT INTO AccountList (ID, CompanyID, BranchID, AccountingType, AccountNumber, AccountName, AccountNameGlobal,
    ...
    SELECT * into #tableParentAccountList from AccountList WHERE CompanyID = @companyID AND IsParentNode = 1;
    ...
    WHILE(@Count > 0)
        BEGIN
            SET @AccountNumber = (SELECT TOP 1 AccountNumber FROM #tableParentAccountList ORDER BY AccountNumber)
            ...
            DELETE FROM #tableParentAccountList WHERE AccountNumber = @AccountNumber AND ID = @ParentAccountID
            SET @Count = @Count - 1
        end
    ...
    INSERT INTO AccountTransfer(ID, CompanyID, AccountingType, AccountTransferCode, AccountTransferOrder, Description,
                                FromAccount, ToAccount, FromAccountData, DebitAccount, CreditAccount, IsActive,
                                IsSecurity)
    SELECT NEWID(),
           @companyID,
           AccountingType,
    ...
    INSERT INTO AutoPrinciple(ID, CompanyID, BranchID, AccountingType, AutoPrincipleName, TypeID, DebitAccount,
    ...
    --ADD DATA BANK
    INSERT INTO Bank(ID, CompanyID, BranchID, BankCode, BankName, BankNameRepresent, Address, Description, IsActive)
    SELECT NEWID(),
           @companyID,
           BranchID,
           BankCode,
    ...
    --ADD DATA CURRENCY
    INSERT INTO Currency(ID, CompanyID, BranchID, CurrencyCode, CurrencyName, ExchangeRate, IsActive, Formula)
    SELECT NEWID(),
           @companyID,
           BranchID,
           CurrencyCode,
    ...
    INSERT INTO FixedAssetCategory(ID, CompanyID, BranchID, FixedAssetCategoryCode, FixedAssetCategoryName, Description,
...
--     DECLARE @Count INT
    ...
    INSERT INTO MaterialGoodsSpecialTaxGroup(ID, CompanyID, BranchID, MaterialGoodsSpecialTaxGroupCode,
                                             MaterialGoodsSpecialTaxGroupName, TaxRate, UnitID, OrderFixCode, ParentID,

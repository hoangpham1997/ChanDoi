IF OBJECT_ID ( 'PROC_BANG_KE_MUA_VAO', 'P' ) IS NOT NULL
    DROP PROCEDURE PROC_BANG_KE_MUA_VAO;
CREATE PROCEDURE [dbo].[PROC_BANG_KE_MUA_VAO]
      (
...
/*end add by cuongpv*/
	...
	Begin
		...
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate , a.AccountingObjectName as accountingObjectName, CompanyTaxCode as companyTaxCode,
		 b.Description as description, b.Amount - b.DiscountAmount  as amount, VATRate as vatRate, VATAmount as vatAmount, VATAccount as vatAccount,1 flag, b.AccountingObjectID as AccountingObjectID
		...
		where a.PostedDate between @FromDate and @ToDate and Recorded = 1 and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2)
		union all
		...
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, a.AccountingObjectName as accountingObjectName, CompanyTaxCode as companyTaxCode,
		 b.Description as description, (0 - Amount) as GiatriHH_chuathue, VATRate as vatRate, (0 - VATAmount) as vatAmount, VATAccount as vatAccount,-1 flag, b.AccountingObjectID as AccountingObjectID
		, a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		...
		union all
		--Hoá đơn mua dịch vụ
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, a.AccountingObjectName as accountingObjectName, a.CompanyTaxCode as companyTaxCode,
		 b.Description as description, b.Amount - b.DiscountAmount as amount, VATRate as vatRate, VATAmount as vatAmount, VATAccount as vatAccount,1 flag, b.AccountingObjectID as AccountingObjectID
		, a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		...
		union all --1.	CHỨNG TỪ NGHIỆP VỤ KHÁC
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, b.AccountingObjectName as accountingObjectName, b.TaxCode as companyTaxCode,
		 b.Description as description, PretaxAmount as amount, VATRate as vatRate, VATAmount as vatAmount, VATAccount as vatAccount,1 flag,NULL as AccountingObjectID
		 , a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		 from GOtherVoucher a join GOtherVoucherDetailTax b on a.ID = b.GOtherVoucherID
		join GoodsServicePurchase c on c.id = b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and Recorded = 1  and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2)
		union all --2.	PHIẾU CHI
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, b.AccountingObjectName as accountingObjectName, b.TaxCode as companyTaxCode,
		 b.Description as description, PretaxAmount as amount, VATRate as vatRate, b.VATAmount as vatAmount, VATAccount as vatAccount	,1 flag,NULL as AccountingObjectID
		, a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		 from MCPayment a join MCPaymentDetailTax b on a.ID = b.MCPaymentID
		join GoodsServicePurchase c on c.id = b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and a.Recorded = 1  and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2)
		union all --3.	UNC/SÉC CK/SÉC TM
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, b.AccountingObjectName as accountingObjectName, b.TaxCode as companyTaxCode,
		 b.Description as description, PretaxAmount as amount, VATRate as vatRate, b.VATAmount as vatAmount, VATAccount as vatAccount,1 flag,NULL as AccountingObjectID
		 , a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		...
		, a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		 from PPInvoice a join PPInvoiceDetail b on a.ID = b.PPInvoiceID
		join GoodsServicePurchase c on c.id = b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and Recorded = 1 and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2) and b.invoiceNo is not null and b.invoiceDate is not null
		union all
		--Trả lại hàng mua, Giảm giá hàng mua --> Cty phải xuất hóa đơn cho người bán --> Giá trị trên mẫu báo cáo là số tiền âm (Mẫu báo cáo theo hình ở dưới)
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, a.AccountingObjectName as accountingObjectName, CompanyTaxCode as companyTaxCode,
		 b.Description as description, (0 - Amount) as GiatriHH_chuathue, VATRate as vatRate, (0 - VATAmount) as vatAmount, VATAccount as vatAccount,-1 flag, b.AccountingObjectID as AccountingObjectID
		...
		from PPDiscountReturn a join PPDiscountReturnDetail b on a.ID = b.PPDiscountReturnID
		join GoodsServicePurchase c on c.ID= b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and Recorded = 1 and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2) and a.invoiceNo is not null and a.invoiceDate is not null
		union all
		--Hoá đơn mua dịch vụ
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, a.AccountingObjectName as accountingObjectName, a.CompanyTaxCode as companyTaxCode,
		 b.Description as description, b.Amount - b.DiscountAmount as amount, VATRate as vatRate, VATAmount as vatAmount, VATAccount as vatAccount,1 flag, b.AccountingObjectID as AccountingObjectID
		...
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, b.AccountingObjectName as accountingObjectName, b.TaxCode as companyTaxCode,
		 ...
		 from GOtherVoucher a join GOtherVoucherDetailTax b on a.ID = b.GOtherVoucherID
		join GoodsServicePurchase c on c.id = b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and Recorded = 1  and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2) and b.InvoiceNo is not null and LEN(b.InvoiceNo) > 0 and b.InvoiceDate is not null
		union all --2.	PHIẾU CHI
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, b.AccountingObjectName as accountingObjectName, b.TaxCode as companyTaxCode,
		 b.Description as description, PretaxAmount as amount, VATRate as vatRate, b.VATAmount as vatAmount, VATAccount as vatAccount	,1 flag,NULL as AccountingObjectID
		, a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		 from MCPayment a join MCPaymentDetailTax b on a.ID = b.MCPaymentID
		join GoodsServicePurchase c on c.id = b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and a.Recorded = 1  and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2) and b.InvoiceNo is not null and LEN(b.InvoiceNo) > 0 and b.InvoiceDate is not null
		union all --3.	UNC/SÉC CK/SÉC TM
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, b.AccountingObjectName as accountingObjectName, b.TaxCode as companyTaxCode,
		 b.Description as description, PretaxAmount as amount, VATRate as vatRate, b.VATAmount as vatAmount, VATAccount as vatAccount,1 flag,NULL as AccountingObjectID
		 , a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
...
-- 			where AccountingObjectID = d.ID;
		...
		SELECT * FROM @tbDataLocal where (vatAmount is not null  and vatAmount  != 0) or (amount is not null  and amount != 0) order by goodsServicePurchaseCode, invoiceDate, invoiceNo;
	...
	Begin
		...
		--Trả lại hàng mua, Giảm giá hàng mua --> Cty phải xuất hóa đơn cho người bán --> Giá trị trên mẫu báo cáo là số tiền âm (Mẫu báo cáo theo hình ở dưới)
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, a.AccountingObjectName as accountingObjectName, CompanyTaxCode as companyTaxCode,
		 a.Reason as description, (0 - Amount) as GiatriHH_chuathue, VATRate as vatRate, ( 0 - VATAmount) as vatAmount, VATAccount as vatAccount,-1 flag, b.AccountingObjectID as AccountingObjectID
		, a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		from PPDiscountReturn a join PPDiscountReturnDetail b on a.ID = b.PPDiscountReturnID
		join GoodsServicePurchase c on c.ID= b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and Recorded = 1 and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2)
		...
		union all --1.	CHỨNG TỪ NGHIỆP VỤ KHÁC
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, b.AccountingObjectName as accountingObjectName, b.TaxCode as companyTaxCode,
		 a.Reason as description, PretaxAmount as amount, VATRate as vatRate, VATAmount as vatAmount, VATAccount as vatAccount,1 flag,NULL as AccountingObjectID
		 , a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		 from GOtherVoucher a join GOtherVoucherDetailTax b on a.ID = b.GOtherVoucherID
		join GoodsServicePurchase c on c.id = b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and Recorded = 1  and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2)
		union all --2.	PHIẾU CHI
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, b.AccountingObjectName as accountingObjectName, b.TaxCode as companyTaxCode,
		 a.Reason as description, PretaxAmount as amount, VATRate as vatRate, b.VATAmount as vatAmount, VATAccount as vatAccount	,1 flag,NULL as AccountingObjectID
		, a.ID as RefID, a.TypeID as TypeID, b.OrderPriority
		 from MCPayment a join MCPaymentDetailTax b on a.ID = b.MCPaymentID
		join GoodsServicePurchase c on c.id = b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and a.Recorded = 1  and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2)
		...
		from PPDiscountReturn a join PPDiscountReturnDetail b on a.ID = b.PPDiscountReturnID
		join GoodsServicePurchase c on c.ID= b.GoodsServicePurchaseID
		where a.PostedDate between @FromDate and @ToDate and Recorded = 1 and a.CompanyID in (select id from Func_getCompany (@CompanyID, @isDependent)) and a.TypeLedger in (@TypeLedger, 2) and  a.invoiceNo is not null and LEN(a.InvoiceNo) > 0 and a.invoiceDate is not null
		union all
		--Hoá đơn mua dịch vụ
		select c.goodsServicePurchaseCode,c.goodsServicePurchaseName, InvoiceNo as invoiceNo, InvoiceDate as invoiceDate, a.AccountingObjectName as accountingObjectName, a.CompanyTaxCode as companyTaxCode,
		 a.Reason as description, Amount - DiscountAmount as amount, VATRate as vatRate, VATAmount as vatAmount, VATAccount as vatAccount ,1 flag, b.AccountingObjectID as AccountingObjectID
...
-- 			where AccountingObjectID = d.ID;
	...
	select aa.goodsServicePurchaseCode,
		   ...
		   --aa.RefID
	from (
		 select * from @tbDataLocal where (amount is not null and amount != 0) or (vatAmount is not null and vatAmount  != 0 )
		) aa

    ...
    begin
        ...
        FROM (
                 ...
                 select a.Date               as ngayCTu,
                        ...
                        isnull((select sum(isnull(CreditAmount, 0))
                                from GeneralLedger g
                                where g.ReferenceID = a.ReferenceID
                                  and g.Account like '3333%'
                                  and g.AccountCorresponding like '511%'), 0) +
                        isnull((select sum(isnull(CreditAmount, 0))
                                from GeneralLedger g
                                where g.ReferenceID = a.ReferenceID
                                  and g.Account like '3331%'
                                  and g.accountCorresponding like '511%'),
                               0)                                as thue,
                        isnull((select sum(isnull(CreditAmount, 0))
                                from GeneralLedger g
                                where g.ReferenceID = a.ReferenceID
                                  and g.Account like '521%'), 0) as khac,
                        c.MaterialGoodsName,
                        c.MaterialGoodsCode,
...
--                    and a.AccountCorresponding like '511%'
--                    and (a.TypeID not in (330, 340))
--                    and (@AccountObjectID is null OR
--                         (a.AccountingObjectID in (select AccountObjectID from #tblListAccountObjectID)))
--                    and (@MaterialGoods is null OR
...
--                           a.ReferenceID, a.TypeID, c.MaterialGoodsCode, c.id
        ...
        FROM (
                 ...
                 select a.Date               as ngayCTu,
                        ...
                        null        as thanhTien,
                        0                    as thue,
                        sum(isnull(DebitAmount, 0) - isnull(CreditAmount, 0))     as khac,
                        c.MaterialGoodsName,
                        c.MaterialGoodsCode,
                        c.id                 as materialGoodsID,
                        a.OrderPriority,

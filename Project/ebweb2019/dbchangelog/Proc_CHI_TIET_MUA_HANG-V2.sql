IF OBJECT_ID ( 'Proc_CHI_TIET_MUA_HANG', 'P' ) IS NOT NULL
    DROP PROCEDURE Proc_CHI_TIET_MUA_HANG;
/*
-- =============================================
-- Edit by:		congnd
    ...
    begin
        ...
        DECLARE @tbDataResult TABLE
                              (
                                  ...
                                  NgayHoaDon      DATE,
                                  MaterialGoodsID UNIQUEIDENTIFIER,
                                  Mahang          NVARCHAR(25),  --MaterialGoodsCode
...
--             Lấy SL mua/đơn giá/giá trị mua/chiết khấu từ Mua hàng
            ...
            SELECT a.AccountingObjectID                                     as AccountObjectID,
                   ...
                   (select sum(COALESCE(ppdrt.Amount, 0)) from PPDiscountReturnDetail ppdrt inner join PPDiscountReturn ppdr on ppdrt.PPDiscountReturnID = ppdr.id where ppdrt.PPInvoiceDetailID = b.id and ppdr.TypeID = 230)          as GiaTriGiamGia          as GiaTriGiamGia
            FROM PPInvoice a

...
BEGIN
    ...
    WHERE A.AccountNumber = [@tblItem].AccountNumber AND A.CompanyID IN (SELECT id FROM Func_getCompany (@CompanyID, @IncludeDependentBranch))
    ...
    FROM dbo.GeneralLedger GL
             ...
             INNER JOIN AccountList ACC ON A.AccountNumber = Acc.AccountNumber AND ACC.CompanyID = GL.CompanyID
    ...
    FROM dbo.FRB03OPNDetailByActivity OPN
             ...
             INNER JOIN AccountList AC ON OPN.AccountNumber = AC.AccountNumber AND AC.CompanyID IN (SELECT id FROM Func_getCompany (@CompanyID, @IncludeDependentBranch)),
         @tblActivity T

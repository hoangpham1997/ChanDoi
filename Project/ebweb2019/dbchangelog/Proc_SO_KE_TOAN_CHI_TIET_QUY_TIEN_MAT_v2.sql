...
-- Proc_SO_KE_TOAN_CHI_TIET_QUY_TIEN_MAT '2493322A-4505-3341-A37A-D16D0E89DE52', '01/01/2020', '12/31/2020','VND',0, ',1112,',1, 1
-- =============================================
    ALTER PROCEDURE [dbo].[Proc_SO_KE_TOAN_CHI_TIET_QUY_TIEN_MAT](@CompanyID UNIQUEIDENTIFIER,
    @FromDate DATETIME,
    @ToDate DATETIME,
    ...
    BEGIN
        ...
        ELSE
            BEGIN
                ...
                WHERE GL.PostedDate <= @ToDate
                  AND Account IN (SELECT * from @tblListAccount)
                  AND CompanyID IN (SELECT CompanyID FROM @tblListCompanyID)
                  AND ((CASE WHEN @CurrencyID = @MainCurrency THEN DebitAmount ELSE DebitAmountOriginal END) > 0 OR
                       (CASE WHEN @CurrencyID = @MainCurrency THEN CreditAmount ELSE CreditAmountOriginal END) > 0)
        ...
        WHILE(@COUNTACC > 0)
            BEGIN
                ...
                SET @ThuDauKy =
                        (SELECT SUM(CASE WHEN @CurrencyID = @MainCurrency THEN DebitAmount ELSE DebitAmountOriginal END)
                         FROM @tbDataGL
                ...
                SET @ChiDauKy = (SELECT SUM(CASE
                                 ...
                                 WHERE PostedDate < @FromDate
                                   ...
                                   AND Account = @Account)
                SET @SoTonDauKy = (CASE WHEN @ThuDauKy <> 0 THEN @ThuDauKy ELSE null END) -
                                  (CASE WHEN @ChiDauKy <> 0 THEN @ChiDauKy ELSE null END)
        ...
        FROM (SELECT ReferenceID                                                                           as RefID,
                     ...
                     (CASE WHEN @CurrencyID = @MainCurrency THEN DebitAmount ELSE DebitAmountOriginal END) as SoTienThu,
                     NULL                                                                                  as SoTienChi,
                     NULL                                                                                  as SoTon,
                     NULL                                                                                  as Note,
                     0                                                                                     as CAType,
                     ...
                     TypeID                                                                                as TypeID
              FROM @tbDataGL
              WHERE (CASE WHEN @CurrencyID = @MainCurrency THEN DebitAmount ELSE DebitAmountOriginal END) > 0

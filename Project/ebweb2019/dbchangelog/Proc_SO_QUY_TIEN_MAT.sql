...
-- =============================================
    CREATE PROCEDURE [dbo].[Proc_SO_QUY_TIEN_MAT](@CompanyID UNIQUEIDENTIFIER,
    @FromDate DATETIME,
    @ToDate DATETIME,
        ...
        ELSE
            BEGIN
                ...
                IF (@ThuDauKy IS NULL)
                    ...
                    END
                SET @ChiDauKy =
                        (SELECT SUM(CASE WHEN @typeShowCurrency = 1 THEN CreditAmount ELSE CreditAmountOriginal END)
                         FROM @tbDataGL
                         WHERE (CASE WHEN @typeShowCurrency = 1 THEN CreditAmount ELSE CreditAmountOriginal END) > 0
                           AND PostedDate < @FromDate)
                IF (@ChiDauKy IS NULL)
                    BEGIN
                        SET @ChiDauKy = 0
            ...
            BEGIN
                ...
                FROM (SELECT ReferenceID   as RefID,
                             ...
                             CASE
                                 ...
                                 ELSE (CASE WHEN @typeShowCurrency = 1 THEN DebitAmount ELSE DebitAmountOriginal END) END
                      ...
                      WHERE (CASE
                                 WHEN @CurrencyID = @MainCurrency THEN CreditAmount
                                 ELSE (CASE WHEN @typeShowCurrency = 1 THEN CreditAmount ELSE CreditAmountOriginal END) END) > 0
                        AND PostedDate >= @FromDate
                        AND PostedDate <= @ToDate
                        AND (TypeLedger = 0 OR TypeLedger = 2)
                     ) as #rs

...
-- Proc_SO_TIEN_GUI_NGAN_HANG 'CAA09998-422C-C742-BF50-F26B2548BAEE', '01/01/2020', '05/22/2020',',1121,1122,1123,','VND',',18b99181-f7d5-4845-a44f-ac574fee8ae0,ceaade31-c029-de47-976a-6285960130a0,263880c9-28b3-f342-900e-3c8c991090f7,',1,0,1,0
-- =============================================
    ...
    BEGIN
        ...
        DECLARE @tblListCompanyID_TKNH TABLE
                                       (
                                           ID UNIQUEIDENTIFIER
                                       )


        INSERT INTO @tblListCompanyID_TKNH exec getCompany @CompanyID, 'TCKHAC_SDDMTKNH'

        DECLARE @tblListCompanyID_TTD TABLE
                                      (
                                          ID UNIQUEIDENTIFIER
                                      )

        INSERT INTO @tblListCompanyID_TTD exec getCompany @CompanyID, 'TCKHAC_SDDMTTD'

--         SELECT * from @tblListCompanyID_TKNH
--         SELECT * from @tblListCompanyID_TTD
...
--         SELECT * from @tblListAccountNumber
        ...
        IF (@isDependent = 1)
            BEGIN
                ...
                INSERT INTO @tblListCompanyID_TKNH
                 SELECT ID
                FROM EbOrganizationUnit
                WHERE (ID = @CompanyID)
                   OR (ParentID = @CompanyID AND UnitType = 1 AND AccType = 0)
                INSERT INTO @tblListCompanyID_TTD
                 SELECT ID
                FROM EbOrganizationUnit
                WHERE (ID = @CompanyID)
                   OR (ParentID = @CompanyID AND UnitType = 1 AND AccType = 0)
        ...
        WHERE BankAccountDetail.Value IS NOT NULL
          AND CompanyID IN (SELECT * from @tblListCompanyID_TKNH)


        INSERT INTO @tblListBankAccountDetail
        SELECT TG.ID
        FROM CreditCard AS TG
                 INNER JOIN Func_ConvertStringIntoTable_Nvarchar(@ListBankAccountDetailID, ',') AS CreditCard
                            ON TG.ID = CreditCard.Value
        WHERE CreditCard.Value IS NOT NULL
          AND CompanyID IN (SELECT * FROM @tblListCompanyID_TTD)
        ...
        FROM (SELECT BankAccountDetailID                                                                          as BankAccountDetailID,
                     ...
                     CASE
                         ...
                         ELSE (CASE
                                   WHEN @typeShowCurrency = 1 THEN DebitAmount
                                   ELSE DebitAmountOriginal END) END                                              as SoThu,
                     NULL                                                                                         as SoChi,
                     NULL                                                                                         as SoTon,
              ...
              WHERE PostedDate >= @FromDate
                ...
                AND (CASE
                         WHEN @CurrencyID = @MainCurrency THEN DebitAmount
                         ELSE (CASE WHEN @typeShowCurrency = 1 THEN DebitAmount ELSE DebitAmountOriginal END) END) > 0
              UNION ALL
              SELECT BankAccountDetailID                                                                            as BankAccountDetailID,
                     ...
                     CASE
                         ...
                         ELSE (CASE
                                   ...
                                   ELSE CreditAmountOriginal END) END                                               as SoChi,
                     NULL                                                                                           as SoTon,
                     OrderPriority                                                                                  as OrderPriority,
                     2                                                                                              as PositionOrder,
                     TypeID                                                                                         as TypeID,
              ...
              WHERE PostedDate >= @FromDate
                ...
                AND (CASE
                         WHEN @CurrencyID = @MainCurrency THEN CreditAmount
                         ELSE (CASE WHEN @typeShowCurrency = 1 THEN CreditAmount ELSE CreditAmountOriginal END) END) >
                    0) as #RS
        ORDER BY OrderPriority
                ...
                SET SoTon = ISNULL(CASE
                                       ...
                                       ELSE (ISNULL((SELECT SUM(CASE
                                                     ...
                                                     WHERE BankAccountDetailID = @BankAccountDetailID
                                                       ...
                                                       AND (CASE
                                                                WHEN @typeShowCurrency = 1 THEN DebitAmount
                                                                ELSE DebitAmountOriginal END) > 0), 0) -
                                             ISNULL((SELECT SUM(CASE
                                                     ...
                                                     WHERE BankAccountDetailID = @BankAccountDetailID
                                                       ...
                                                       AND CASE
                                                               WHEN @typeShowCurrency = 1 THEN CreditAmount
                                                               ELSE CreditAmountOriginal END > 0), 0)) END, 0)

...
-- =============================================
    CREATE PROCEDURE [dbo].[Proc_SO_TIEN_GUI_NGAN_HANG](@CompanyID UNIQUEIDENTIFIER,
    @FromDate DATETIME,
    @ToDate DATETIME,
    ...
    BEGIN
        ...
        ELSE
            BEGIN
                ...
                WHERE GL.PostedDate <= @ToDate
                  AND Account IN (SELECT AccountNumber FROM @tblListAccountNumber)
                  AND CompanyID IN (SELECT CompanyID FROM @tblListCompanyID)
                  AND ((CASE WHEN @typeShowCurrency = 1 THEN DebitAmount ELSE DebitAmountOriginal END) > 0 OR (CASE WHEN @typeShowCurrency = 1 THEN CreditAmount ELSE CreditAmountOriginal END) > 0)
                  AND (TypeLedger = 2 OR TypeLedger = @TypeLedger)
                  AND BankAccountDetailID IN (SELECT BankAccountDetailID FROM @tblListBankAccountDetail)
        ...
        FROM (SELECT BankAccountDetailID                                                                 as BankAccountDetailID,
                     ...
                     CASE WHEN @CurrencyID = @MainCurrency THEN DebitAmount ELSE (CASE WHEN @typeShowCurrency = 1 THEN DebitAmount ELSE DebitAmountOriginal END) END as SoThu,
                     NULL                                                                                as SoChi,
                     NULL                                                                                as SoTon,
              ...
              WHERE PostedDate >= @FromDate
                ...
                AND (CASE WHEN @CurrencyID = @MainCurrency THEN DebitAmount ELSE (CASE WHEN @typeShowCurrency = 1 THEN DebitAmount ELSE DebitAmountOriginal END) END) > 0
              UNION ALL
              SELECT BankAccountDetailID                                                                   as BankAccountDetailID,
                     ...
                     AccountCorresponding                                                                  as AccounntCorresponding,
                     NULL                                                                                  as SoThu,
                     CASE WHEN @CurrencyID = @MainCurrency THEN CreditAmount ELSE (CASE WHEN @typeShowCurrency = 1 THEN CreditAmount ELSE CreditAmountOriginal END) END as SoChi,
                     NULL                                                                                  as SoTon,
                     OrderPriority                                                                         as OrderPriority,
                     2                                                                                     as PositionOrder,
                     TypeID                                                                                as TypeID,
              ...
              WHERE PostedDate >= @FromDate
                ...
                AND (CASE
                         ...
                         ELSE (CASE WHEN @typeShowCurrency = 1 THEN CreditAmount ELSE CreditAmountOriginal END) END) > 0) as #RS
        ORDER BY OrderPriority
        ...
        WHILE(@COUNT > 0)
            BEGIN
                ...
                SET SoTon = ISNULL(CASE
                                       ...
                                       ELSE (ISNULL((SELECT SUM(CASE WHEN @typeShowCurrency = 1 THEN DebitAmount ELSE DebitAmountOriginal END)
                                                     FROM @tbDataGL
                                                     WHERE BankAccountDetailID = @BankAccountDetailID
                                                       AND PostedDate < @FromDate
                                                       AND (CASE WHEN @typeShowCurrency = 1 THEN DebitAmount ELSE DebitAmountOriginal END) > 0), 0) -
                                             ISNULL((SELECT SUM(CASE WHEN @typeShowCurrency = 1 THEN CreditAmount ELSE CreditAmountOriginal END)
                                                     FROM @tbDataGL
                                                     WHERE BankAccountDetailID = @BankAccountDetailID
                                                       AND PostedDate < @FromDate
                                                       AND CASE WHEN @typeShowCurrency = 1 THEN CreditAmount ELSE CreditAmountOriginal END > 0), 0)) END, 0)

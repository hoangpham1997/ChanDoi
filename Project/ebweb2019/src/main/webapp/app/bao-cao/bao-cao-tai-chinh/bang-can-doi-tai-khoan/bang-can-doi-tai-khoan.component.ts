...
@Component({
    ...
    templateUrl: './bang-can-doi-tai-khoan.component.html',
    styleUrls: ['./bang-can-doi-tai-khoan.component.css']
})
export class BangCanDoiTaiKhoanComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.principal.identity().then(account => {
            ...
            // });
            this.accountListService.getGradeAccount().subscribe((res: HttpResponse<any[]>) => {
                if (res.body.length > 0) {
                    res.body.forEach(item => {
                        this.grades.push({ value: item });
                    });
                    this.grade = res.body[0];
                }
            });
        });
    }
    ...
    getSessionData() {
        this.dataSession = JSON.parse(sessionStorage.getItem('searchBangCanDoiTaiKhoan'));
        if (this.dataSession) {
    ...
    setSessionData() {
        ...
        sessionStorage.setItem('searchBangCanDoiTaiKhoan', JSON.stringify(this.dataSession));
    }
    ...
    showReport() {
        if (this.checkErr()) {
            ...
            this.requestReport.option = this.option;
            this.requestReport.grade = this.grade;
            this.requestReport.dependent = this.similarBranch;
            this.requestReport.typeReport = BaoCao.BaoCaoTaiChinh.BANG_CAN_DOI_TAI_KHOAN;
    ...
    exportExcel() {
        if (this.checkErr()) {
            ...
            this.requestReport.dependent = this.similarBranch;
            this.requestReport.typeReport = BaoCao.BaoCaoTaiChinh.BANG_CAN_DOI_TAI_KHOAN;
            this.requestReport.fileName = BaoCao.BaoCaoTaiChinh.BANG_CAN_DOI_TAI_KHOAN_XLS;
            this.baoCaoService.exportExcel(this.requestReport).subscribe(
                res => {
                    ...
                    const name = BaoCao.BaoCaoTaiChinh.BANG_CAN_DOI_TAI_KHOAN_XLS;
    ...
    checkErr() {
        ...
        if (!this.grade) {
            this.toastr.error(
                this.translate.instant('ebwebApp.baoCao.canDoiTaiKhoan.nullGrade'),
                this.translate.instant('ebwebApp.mCReceipt.error.error')
            );
            return false;
        }

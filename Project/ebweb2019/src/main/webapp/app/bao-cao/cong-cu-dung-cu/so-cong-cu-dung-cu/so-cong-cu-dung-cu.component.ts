...
@Component({
    selector: 'eb-so-cong-cu-dung-cu',
    templateUrl: './so-cong-cu-dung-cu.component.html',
    styleUrls: ['so-cong-cu-dung-cu.component.css']
})
export class SoCongCuDungCuComponent implements OnInit {
    createDate: Moment;
    fromDate: Moment;
    toDate: Moment;
    ...
    isKetXuat: any;
    ROLE_KetXuat = ROLE.SoCongCuDungCu_KetXuat;
    ...
    ngOnInit(): void {
        ...
        this.principal.identity().then(account => {
            ...
            this.getSessionData();
            this.getAllToolsByCompanyID();
        });
    }

    getSessionData() {
        this.dataSession = JSON.parse(sessionStorage.getItem('searchCCDC'));
        if (this.dataSession) {
    ...
    setSessionData() {
        ...
        sessionStorage.setItem('searchCCDC', JSON.stringify(this.dataSession));
    }

    getAllToolsByCompanyID() {
        ...
        this.toolsService
            ...
            .subscribe((res: HttpResponse<ITools[]>) => {
                this.toolsAll = res.body;
    ...
    showReport() {
        if (this.checkErr()) {
            ...
            this.requestReport.typeReport = BaoCao.CCDC.SO_CONG_CU_DUNG_CU;
    ...
    exportExcel() {
        if (this.checkErr()) {
            ...
            // this.requestReport.groupTheSameItem = this.groupTheSameItem;
            this.requestReport.typeReport = BaoCao.CCDC.SO_CONG_CU_DUNG_CU;
            this.requestReport.fileName = BaoCao.CCDC.SO_CONG_CU_DUNG_CU_XLS;
            this.requestReport.companyID = this.organizationUnit.parent.id;
            ...
            this.baoCaoService.exportExcel(this.requestReport).subscribe(
                res => {
                    ...
                    const name = BaoCao.CCDC.SO_CONG_CU_DUNG_CU_XLS;
    ...
    checkErr() {
        ...
        if (!this.tools.some(n => n.checked)) {
            this.toastr.error(
                this.translate.instant('ebwebApp.baoCao.error.nullTool'),
                this.translate.instant('ebwebApp.mCReceipt.error.error')
            );
            return false;
        }

...
@Component({
    ...
    templateUrl: './so-chi-tiet-vat-lieu.component.html',
    styleUrls: ['./so-chi-tiet-vat-lieu.component.css']
})
export class SoChiTietVatLieuComponent implements OnInit {
    ...
    mgCodeFilter: string;
    mgNameFilter: string;
    ROLE_KetXuat = ROLE.SoChiTietVatLieu_KetXuat;
    isKetXuat: boolean;
    constructor(
    ...
    getSessionData() {
        this.dataSession = JSON.parse(sessionStorage.getItem('searchSoChiTietVatLieu'));
        if (this.dataSession) {
    ...
    setSessionData() {
        ...
        sessionStorage.setItem('searchSoChiTietVatLieu', JSON.stringify(this.dataSession));
    }
    ...
    showReport() {
        if (this.checkErr()) {
            ...
            this.requestReport.typeReport = BaoCao.Kho.SO_CHI_TIET_VAT_LIEU;
    ...
    exportExcel() {
        if (this.checkErr()) {
            ...
            this.requestReport.listMaterialGoods = this.materialGoods.filter(x => x.checked).map(c => c.id);
            this.requestReport.typeReport = BaoCao.Kho.SO_CHI_TIET_VAT_LIEU;
            this.requestReport.fileName = BaoCao.Kho.SO_CHI_TIET_VAT_LIEU_XLS;
            this.requestReport.dependent = this.similarBranch;
            this.baoCaoService.exportExcel(this.requestReport).subscribe(
                res => {
                    ...
                    const name = BaoCao.Kho.SO_CHI_TIET_VAT_LIEU_XLS;

...
type EntityResponseType = HttpResponse<ITIAdjustment>;
type EntityMutipleRecord = HttpResponse<IMultipleRecord>;
type EntityArrayResponseType = HttpResponse<ITIAdjustment[]>;

@Injectable({ providedIn: 'root' })
export class DieuChinhCcdcService {
    private resourceUrl = SERVER_API_URL + 'api/t-i-adjustments';

    constructor(private http: HttpClient) {}

    create(itiAdjustment: ITIAdjustment): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(itiAdjustment);
        return this.http
            .post<IGOtherVoucher>(this.resourceUrl, copy, { observe: 'response' })
            .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
    }

    update(itiAdjustment: ITIAdjustment): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(itiAdjustment);
        return this.http
    ...
    private convertDateFromClient(itiAdjustment: ITIAdjustment): ITIAdjustment {
        const copy: ITIAdjustment = Object.assign({}, itiAdjustment, {
            date: itiAdjustment.date != null && itiAdjustment.date.isValid() ? itiAdjustment.date.format(DATE_FORMAT) : null,
            postedDate:
                itiAdjustment.postedDate != null && itiAdjustment.postedDate.isValid() ? itiAdjustment.postedDate.format(DATE_FORMAT) : null
        });
        return copy;
    }

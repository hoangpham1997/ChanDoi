...
@Component({
    selector: 'eb-dieu-chuyen-ccdc',
    templateUrl: './dieu-chuyen-ccdc.component.html',
    styleUrls: ['./dieu-chuyen-ccdc.component.css']
})
export class DieuChuyenCcdcComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    currentAccount: any;
    itiTransfers: ITITransfer[];
    itiTransfersDetails: ITITransferDetails[];
    totalItems: any;
    queryCount: any;
    itemsPerPage: any;
    page: any;
                ...
                res => {
                    ...
                    if (res.body.length > 0) {
                        if (JSON.parse(sessionStorage.getItem('itemGOtherVoucher'))) {
                            ...
                            this.selectedRow = this.itiTransfers.find(item => item.id === itemID);
                            if (!this.selectedRow) {
                                // this.selectedRow = this.itiTransfers[0];
                                this.onSelect(this.itiTransfers[0]);
                            } else {
    ...
    export() {
        event.preventDefault();
        if (!(!this.itiTransfers || this.itiTransfers.length === 0 || !this.selectedRow)) {
            this.phanBoCcdcService
                ...
                .subscribe(res => {
                    this.refModalService.open(null, EbReportPdfPopupComponent, res, false, TypeID.DIEU_CHUYEN_CCDC);
                });
        }
    }
    ...
    unrecord() {
        ...
        if (
            ...
            if (this.selectedRows && this.selectedRows.length > 1) {
                ...
                if (
                    !(this.itiTransfersDetails && this.itiTransfersDetails.length < 1) ||
    ...
    doubleClickRow(id: string, check?: boolean) {
        if (id) {
            ...
            this.router.navigate(['./dieu-chuyen-ccdc', id, 'edit']);
        }
    }

    deleteAfter() {
        if (this.selectedRows && this.selectedRows.length > 1) {
            this.phanBoCcdcService.multiDeletePB(this.selectedRows).subscribe(
                res => {
                    if (this.modalRef) {
                        this.modalRef.close();
                    }
            ...
            if (this.selectedRow && this.selectedRow.id) {
                this.dieuChuyenCcdcService.delete(this.selectedRow.id).subscribe(
                    res => {
                        this.toastr.success(this.translate.instant('ebwebApp.pPDiscountReturn.delete.success'));
                        if (this.modalRef) {
                            this.modalRef.close();
    ...
    registerExport() {
        this.eventSubscriber = this.eventManager.subscribe(`export-excel-${TypeID.DIEU_CHUYEN_CCDC}`, () => {
            this.exportExcel();
        });
    }
    ...
    onSelect(item: IGOtherVoucher) {
        ...
        this.index = this.itiTransfers.indexOf(this.selectedRow);
        this.rowNum = this.getRowNumberOfRecord(this.page, this.index);
        if (item.id) {
            if (item.id) {
                this.dieuChuyenCcdcService.findDetailViewPB(this.selectedRow.id).subscribe(res => {
                    this.itiTransfersDetails = res.body.tiTransferDetailsConvertDTOS;
    ...
    delete() {
        ...
        if (
            ...
            if (this.selectedRows && this.selectedRows.length > 1) {
                ...
                // if (
                //     !(
                //         (this.itiTransfers && this.itiTransfers.length === 0) ||
                //         !this.selectedRow ||
                //         (this.selectedRow.recorded && this.selectedRows.length <= 1) ||
                //         this.checkCloseBook(this.currentAccount, this.selectedRow.postedDate)
                //     )
    ...
    sumAllList() {
        ...
        for (let i = 0; i < this.itiTransfersDetails.length; i++) {
            if (this.itiTransfersDetails[i].quantity) {
                this.sumQuantity += this.itiTransfersDetails[i].quantity;
            }
            if (this.itiTransfersDetails[i].transferQuantity) {
                this.sumTransferQuantity += this.itiTransfersDetails[i].transferQuantity;
            }
        }
    }
    ...
    continueMultiAction() {
        if (this.typeMultiAction === 1) {
            ...
            listRecord.typeIDMain = TypeID.DIEU_CHUYEN_CCDC;
            listRecord.records = [];

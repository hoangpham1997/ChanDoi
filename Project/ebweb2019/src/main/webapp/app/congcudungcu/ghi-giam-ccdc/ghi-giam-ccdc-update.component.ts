...
import { ITIDecrement } from 'app/shared/model/ti-decrement.model';
...
@Component({
    selector: 'eb-ghi-giam-ccdc-update',
    templateUrl: './ghi-giam-ccdc-update.component.html',
    styleUrls: ['./ghi-giam-ccdc-update.component.css']
})
export class GhiGiamCcdcUpdateComponent extends BaseComponent implements OnInit, AfterViewInit, AfterViewChecked {
    ...
    private _itiDecrement: ITIDecrement;
    isSaving: boolean;
    totalAmount: number;
    ...
    noCopy: any;
    REPORT = REPORT;
    sumRemainingDecrementAmount: any;
    sumDecrementQuantity: any;
    sumQuantity: any;
    ...
    tiDecrementDetail: TIDecrementDetails[];
    tiDecrementDetailCopy: TIDecrementDetails[];
    tiDecrementCopy: any;
    listTools2: any;
    ...
    constructor(
        ...
        private ghiGiamCcdcService: GhiGiamCcdcService,
        ...
        this.principal.identity().then(account => {
            ...
            this.activatedRoute.data.subscribe(data => {
                if (data.tiDecrement && data.tiDecrement.id) {
                    this.checkData = true;
                    this.isEdit = false;
                    this.tiDecrement = data.tiDecrement;
                    this.tiDecrement.date = moment(data.tiDecrement.date);
                    // this.tiDecrement.postedDate = moment(data.tiAllocation.postedDate);
                    this.tiDecrementDetail = data.tiDecrement.tiDecrementDetails;
                    if (this.isSoTaiChinh) {
                        this.no = this.tiDecrement.noFBook;
                    } else if (this.sysTypeLedger === 1) {
                        this.no = this.tiDecrement.noMBook;
                    }
                    if (this.dataSession && this.dataSession.isEdit) {
                        this.edit();
                    }
                    this.pPDiscountReturnService.getRefVouchersByPPdiscountReturnID(this.tiDecrement.id).subscribe(ref => {
                    ...
                    this.isEdit = true;
                    this.tiDecrement = {};
                    this.tiDecrement.date = this.utilsService.ngayHachToan(this.currentAccount);
                    // this.tiDecrement.postedDate = this.tiDecrement.date;
                    this.tiDecrementDetail = [];
    ...
    ngOnInit() {
        ...
        this.sumAllList();
        // add by namnh
        if (window.location.href.includes('/from-ghi-giam-ccdc')) {
            this.isGOtherVoucher = true;
            this.searchVoucher = JSON.parse(sessionStorage.getItem('dataSearchGhiGiamCCDC'));
            this.utilsService
                .getIndexRow({
                    id: this.tiDecrement.id,
                    isNext: true,
    ...
    get tiDecrement() {
        return this._itiDecrement;
    }

    set tiDecrement(itiDecrement: ITIDecrement) {
        this._itiDecrement = itiDecrement;
    }
    ...
    getData() {
        ...
        this.autoPrinciplellService.getAllAutoPrinciples().subscribe((res: HttpResponse<IAutoPrinciple[]>) => {
            ...
            if (this.tiDecrement.id) {
                gOtherVoucherReason.autoPrincipleName = this.tiDecrement.reason;
                if (this.autoPrinciples && !this.autoPrinciples.some(x => x.autoPrincipleName === this.tiDecrement.reason)) {
                    this.autoPrinciples.push(gOtherVoucherReason);
                }
            } else {
                //     if (this.objectDate) {
                const dateItem = this.tiDecrement.date;
                this.translate.get(['ebwebApp.gOtherVoucher.reasonTitle']).subscribe((ref: any) => {
                    this.tiDecrement.reason = this.translate.instant('ebwebApp.tIDecrement.reason', {
                        date: dateItem
    ...
    canDeactive() {
        if (this.isCloseAll) {
            ...
            return (
                this.utilService.isEquivalent(this.tiDecrement, this.tiDecrementCopy) &&
                ...
                this.utilService.isEquivalentArray(this.tiDecrementDetail, this.tiDecrementDetailCopy)
            );
        }
    }
    ...
    addNewRow(eventData: any, select: number) {
        ...
        this.tiDecrementDetail.push({});
        this.changeAllocationAmount();

        const nameTag: string = event.srcElement.id;
        const index: number = this.tiDecrementDetail.length - 1;
        ...
        if (index !== null && index >= 0) {
            if (screen === 0) {
                switch (key) {
                    case 'ctr + delete': {
                        const idByIndex = '';
                        this.tiDecrementDetail.splice(index, 1);
                        const lst = this.listIDInputDeatilTax;
                        const col = this.indexFocusDetailCol;
                        const row = this.indexFocusDetailRow - 1;
                        setTimeout(function() {
                    ...
                    case 'ctr + c': {
                        ...
                        detailCopy.id = null;
                        this.tiDecrementDetail.splice(index + 1, 0, detailCopy);
                        const lst = this.listIDInputDeatilTax;
                        const col = this.indexFocusDetailCol;
                        const row = this.indexFocusDetailRow + 1;
                        setTimeout(function() {
                    ...
                    case 'ctr + insert': {
                        ...
                        const lst = this.listIDInputDeatilTax;
                        const col = this.indexFocusDetailCol;
                        const row = this.tiDecrementDetail.length - 1;
                        setTimeout(function() {
                            const element: HTMLElement = document.getElementById(lst[col] + row);
                            if (element) {
                                element.focus();
    ...
    copy() {
        this.tiDecrementCopy = Object.assign({}, this.tiDecrement);
        this.noCopy = this.no;
        ...
        if (this.tiDecrementDetail) {
            this.tiDecrementDetailCopy = this.tiDecrementDetail.map(object => ({ ...object }));
        }
    }
    ...
    //     // if (this.objectDate) {
    //         // this.translate.get(['ebwebApp.gOtherVoucher.reasonTitle']).subscribe((ref: any) => {
    //         //     this.tiDecrement.reason = this.translate.instant('ebwebApp.gOtherVoucher.reasonTitle', {
    ...
    //         //     }
    //         // });
    //         this.tiDecrement.date = moment(new Date(this.objectDate.year, this.objectDate.month, 0));
    //         // this.tiDecrement.postedDate = moment(new Date(this.objectDate.year, this.objectDate.month, 0));
    //         this.ghiGiamCcdcService
    //             .getDetails({
    //                 month: this.objectDate.month,
    //                 year: this.objectDate.year
    //             })
    ...
    //                 this.tiDecrementDetail = res.body;
    //                 // this.tiDecrementDetail.forEach(x => {
    ...
    //                 //     itiAllocationPost.description = this.tiDecrement.reason;
    ...
    //                 //     } else {
    //                 //          this.tiAllocationPosts[i].description = this.tiDecrement.reason;
    //                 //     }
    ...
    getNoBook() {
        this.utilService
            .getGenCodeVoucher({
                typeGroupID: 53, // typeGroupID loại chứng từ
            ...
            .subscribe((res: HttpResponse<string>) => {
                ...
                if (!this.checkData) {
                    this.no = res.body;
                    if (this.isSoTaiChinh) {
                        this.tiDecrement.noFBook = this.no;
                    } else {
                        this.tiDecrement.noMBook = this.no;
                    }
                }
                this.copy();
            });
    ...
    saveAfter(check?: Boolean) {
        for (let i = 0; i < this.tiDecrementDetail.length; i++) {
            this.tiDecrementDetail[i].orderPriority = i;
        }
        // this.tiDecrement.tiAllocationAllocateds = this.tiAllocationAllocateds;
        // this.tiDecrement.tiAllocationDetails = this.tiDecrementDetail;
        // this.tiDecrement.tiAllocationPosts = this.tiAllocationPosts;
        if (this.isSoTaiChinh) {
            this.tiDecrement.noFBook = this.no;
        } else {
            this.tiDecrement.noMBook = this.no;
        }
        this.tiDecrement.totalAmount = this.sumRemainingDecrementAmount;
        this.tiDecrement.tiDecrementDetails = this.tiDecrementDetail;
        this.tiDecrement.viewVouchers = this.viewVouchersSelected;
        if (this.tiDecrement.id) {
            this.ghiGiamCcdcService.update(this.tiDecrement).subscribe(
                res => {
                    this.tiDecrement = res.body;
                    this.copy();
                    ...
                    if (check) {
                        ...
                        this.isEdit = false;
                        // this.router.navigate(['./ghi-giam-ccdc', res.body.gOtherVoucher.id, 'edit']);
                        this.ngOnInit();
                    }
                },
                res => {
            ...
            this.ghiGiamCcdcService.create(this.tiDecrement).subscribe(
                res => {
                    this.tiDecrement = res.body;
                    this.copy();
                    this.isCloseAll = true;
                    ...
                    if (check) {
                        ...
                        this.router.navigate(['./ghi-giam-ccdc', res.body.id, 'edit']);
                    }
                },
                res => {
    ...
    closeAll() {
        this.isCloseAll = true;
        if (sessionStorage.getItem('dataSourceGhiGiamCCDC')) {
            this.router.navigate(['/ghi-giam-ccdc', 'hasSearch', '1']);
        } else if (window.location.href.includes('/from-ghi-giam-ccdc')) {
            if (JSON.parse(sessionStorage.getItem('dataSearchGhiGiamCCDC')) !== null) {
                this.router.navigate(['/ghi-giam-ccdc', 'hasSearch']);
            } else {
                this.router.navigate(['/ghi-giam-ccdc']);
            }
        } else {
            this.router.navigate(['/ghi-giam-ccdc']);
        }
    ...
    addNew($event) {
        $event.preventDefault();
        this.router.navigate(['./ghi-giam-ccdc/new']);
    ...
    edit() {
        event.preventDefault();
        if (
            !(this.isEdit || this.tiDecrement.recorded) &&
            !this.checkCloseBook(this.currentAccount, this.tiDecrement.date) &&
            !this.utilsService.isShowPopup
        ) {
            this.isEdit = true;
            this.listTools = this.listTools2;
            this.copy();
        }
    }
    ...
    record() {
        event.preventDefault();
        // if (!(this.tiDecrement || this.checkCloseBook(this.currentAccount, this.tiDecrement.date)) && !this.utilsService.isShowPopup) {
        if (!this.tiDecrement.recorded && !this.utilsService.isShowPopup) {
            if (this.tiDecrement.id) {
                this.record_ = {};
                this.record_.id = this.tiDecrement.id;
                this.record_.typeID = TypeID.GHI_GIAM_CCDC;
                if (!this.tiDecrement.recorded) {
                    this.gLService.record(this.record_).subscribe((res: HttpResponse<Irecord>) => {
                        if (res.body.success) {
                            this.tiDecrement.recorded = true;
                            this.toastr.success(
    ...
    unrecord() {
        event.preventDefault();
        // if (!(this.isEdit || this.checkCloseBook(this.currentAccount, this.tiDecrement.date))) {
        if (this.tiDecrement.recorded) {
            if (this.tiDecrement.id) {
                this.record_ = {};
                this.record_.id = this.tiDecrement.id;
                ...
                this.gLService.unrecord(this.record_).subscribe((res: HttpResponse<Irecord>) => {
                    if (res.body.success) {
                        this.tiDecrement.recorded = false;
                        this.toastr.success(
    ...
    deleteAfter() {
        if (this.tiDecrement && this.tiDecrement.id) {
            this.ghiGiamCcdcService.delete(this.tiDecrement.id).subscribe(
                res => {
    ...
    deleteRow(index) {
        if (index !== null && index >= 0) {
            ...
            //     for (let i = 0; i < this.tiDecrementDetail.length; i++) {
            //         if (this.tiDecrementDetail[i].prepaidExpenseID ===  this.tiAllocationAllocateds[index].prepaidExpenseID) {
            //             this.tiDecrementDetail.splice(i, 1);
            //             break;
            //         }
            //     }
            // }
    ...
    save(check?: boolean) {
        ...
        if (this.isEdit) {
            ...
            for (let i = 0; i < this.tiDecrementDetail.length; i++) {
                this.tiDecrementDetail[i].orderPriority = i;
                if (this.tiDecrementDetail[i].quantity < this.tiDecrementDetail[i].decrementQuantity) {
                    this.toastr.error(
                        this.translate.instant('ebwebApp.tIDecrementDetails.error.quantityMax'),
                        this.translate.instant('ebwebApp.mCReceipt.error.error')
                    );
                    return false;
                }
            ...
            if (this.isEdit && !this.utilsService.isShowPopup) {
                ...
                if (!this.tiDecrementDetail || this.tiDecrementDetail.length <= 0) {
                    this.toastr.error(this.translate.instant('ebwebApp.pPDiscountReturn.notPPDiscountReturn'));
                    return;
                }
                if (this.checkCloseBook(this.currentAccount, this.tiDecrement.date)) {
                    this.toastr.error(
                ...
                this.tiDecrement.totalAmount = 0;
    ...
    move(direction: number) {
        ...
        return this.ghiGiamCcdcService
            .findByRowNumPB({
    ...
    delete() {
        event.preventDefault();
        if (!(this.isEdit || this.tiDecrement.recorded) && !this.utilsService.isShowPopup) {
            if (this.tiDecrement.id) {
                // this.ghiGiamCcdcService.getMaxMonth(this.tiDecrement.id).subscribe(res => {
                //     if (moment(this.tiDecrement.date).isSame(moment(res.body))) {
    ...
    changeTools(detail: any) {
        if (detail.toolsItem) {
            // const prepaidItem = this.tiDecrementDetails.find(i => i. === detail.prepaidExpenseID);
    ...
    sumAllList() {
        this.sumRemainingDecrementAmount = 0;
        this.sumDecrementQuantity = 0;
        this.sumQuantity = 0;
        for (let i = 0; i < this.tiDecrementDetail.length; i++) {
            if (this.tiDecrementDetail[i].remainingDecrementAmount) {
                this.sumRemainingDecrementAmount += this.tiDecrementDetail[i].remainingDecrementAmount;
            }
            if (this.tiDecrementDetail[i].decrementQuantity) {
                this.sumDecrementQuantity += this.tiDecrementDetail[i].decrementQuantity;
            }
            if (this.tiDecrementDetail[i].quantity) {
                this.sumQuantity += this.tiDecrementDetail[i].quantity;
            }
        }
        this.tiDecrement.totalAmount = this.sumRemainingDecrementAmount;
    }
    ...
    navigate(iGOtherVoucher: IGOtherVoucher) {
        switch (iGOtherVoucher.typeID) {
            case this.TYPE_CHUNG_TU_NGHIEP_VU_KHAC:
                this.router.navigate(['/ghi-giam-ccdc', iGOtherVoucher.id, 'edit']);
                break;
            case this.TYPE_KET_CHUYEN_LAI_LO:
                this.router.navigate(['./ket-chuyen-lai-lo', iGOtherVoucher.id, 'edit', 'from-ghi-giam-ccdc']);
                break;
            case TypeID.GHI_GIAM_CCDC:
                this.router.navigate(['./ghi-giam-ccdc', iGOtherVoucher.id, 'edit', 'from-ghi-giam-ccdc']);
                break;
        }
    }
    ...
    getToolsActive() {
        if (this.tiDecrement && this.tiDecrement.id) {
            this.ghiTangService.getToolsActive().subscribe(res => {
                ...
                if (this.tiDecrementDetail) {
                    this.tiDecrementDetail.forEach(item => {
        ...
        this.ghiTangService
            .getToolsActiveByTIDecrement({
                date: this.tiDecrement
                    ? this.tiDecrement.date instanceof moment
                        ? this.tiDecrement.date.format(DATE_FORMAT_SEARCH)
                        : moment(this.tiDecrement.date).format(DATE_FORMAT_SEARCH)
                    : ''
            })
            .subscribe(res => {
                if (!this.tiDecrement || !this.tiDecrement.id) {
    ...
    changeOrganizationUnits(detail: any) {
        detail.quantity = detail.organizationUnits.find(x => x.id === detail.departmentID).quantityRest;
    }
}

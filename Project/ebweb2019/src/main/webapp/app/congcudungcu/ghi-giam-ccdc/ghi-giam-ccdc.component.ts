...
import { ITIDecrementDetails } from 'app/shared/model/ti-decrement-details.model';

@Component({
    selector: 'eb-dieu-chinh-ccdc',
    templateUrl: './ghi-giam-ccdc.component.html',
    styleUrls: ['./ghi-giam-ccdc.component.css']
})
export class GhiGiamCcdcComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    currentAccount: any;
    itiDecrements: ITIDecrement[];
    itiDecrementDetails: ITIDecrementDetails[];
    totalItems: any;
    queryCount: any;
    itemsPerPage: any;
    page: any;
    ...
    search() {
        ...
        this.ghiGiamCcdcService
            ...
            .subscribe(
                res => {
                    ...
                    this.itiDecrements = res.body;
                    console.log(this.itiDecrements);
                    ...
                    if (res.body.length > 0) {
                        if (JSON.parse(sessionStorage.getItem('itemGOtherVoucher'))) {
                            ...
                            this.selectedRow = this.itiDecrements.find(item => item.id === itemID);
                            if (!this.selectedRow) {
                                // this.selectedRow = this.itiDecrements[0];
                                this.onSelect(this.itiDecrements[0]);
                            } else {
        ...
        sessionStorage.setItem('dataSearchGhiGiamCCDC', JSON.stringify(this.objSearch));
    }
    ...
    export() {
        event.preventDefault();
        if (!(!this.itiDecrements || this.itiDecrements.length === 0 || !this.selectedRow)) {
            this.phanBoCcdcService
                ...
                .subscribe(res => {
                    this.refModalService.open(null, EbReportPdfPopupComponent, res, false, TypeID.GHI_GIAM_CCDC);
                });
        }
    }
    ...
    doubleClickRow(id: string, check?: boolean) {
        if (id) {
            ...
            this.router.navigate(['./ghi-giam-ccdc', id, 'edit']);
        }
    }

    deleteAfter() {
        if (this.selectedRows && this.selectedRows.length > 1) {
            this.ghiGiamCcdcService.multiDeletePB(this.selectedRows).subscribe(
                res => {
                    if (this.modalRef) {
                        this.modalRef.close();
                    }
            ...
            if (this.selectedRow && this.selectedRow.id) {
                this.ghiGiamCcdcService.delete(this.selectedRow.id).subscribe(
                    res => {
                        this.toastr.success(this.translate.instant('ebwebApp.pPDiscountReturn.delete.success'));
                        if (this.modalRef) {
                            this.modalRef.close();
    ...
    registerExport() {
        this.eventSubscriber = this.eventManager.subscribe(`export-excel-${TypeID.GHI_GIAM_CCDC}`, () => {
            this.exportExcel();
        });
    }
    ...
    onSelect(item: IGOtherVoucher) {
        ...
        this.index = this.itiDecrements.indexOf(this.selectedRow);
        this.rowNum = this.getRowNumberOfRecord(this.page, this.index);
        if (item.id) {
            this.ghiGiamCcdcService.findDetailPB(this.selectedRow.id).subscribe(res => {
                this.itiDecrementDetails = res.body.tiDecrementDetailsConvertDTOS;
    ...
    delete() {
        ...
        if (
            ...
            if (this.selectedRows && this.selectedRows.length > 1) {
                ...
                // if (
                //     !(
                //         (this.itiDecrements && this.itiDecrements.length === 0) ||
                //         !this.selectedRow ||
                //         (this.selectedRow.recorded && this.selectedRows.length <= 1) ||
                //         this.checkCloseBook(this.currentAccount, this.selectedRow.postedDate)
                //     )
                // ) {
                if (this.selectedRow.id) {
                    // this.phanBoCcdcService.getMaxMonth(this.selectedRow.id).subscribe(res => {
    ...
    sumAllList() {
        ...
        this.sumQuantity = 0;
        for (let i = 0; i < this.itiDecrementDetails.length; i++) {
            if (this.itiDecrementDetails[i].remainingDecrementAmount) {
                this.sumRemainingDecrementAmount += this.itiDecrementDetails[i].remainingDecrementAmount;
            }
            if (this.itiDecrementDetails[i].decrementQuantity) {
                this.sumDecrementQuantity += this.itiDecrementDetails[i].decrementQuantity;
            }
            if (this.itiDecrementDetails[i].quantity) {
                this.sumQuantity += this.itiDecrementDetails[i].quantity;
            }
        }
    }
    ...
    continueMultiAction() {
        if (this.typeMultiAction === 1) {
            this.ghiGiamCcdcService.multiUnRecord(this.selectedRows).subscribe(
            ...
            listRecord.typeIDMain = TypeID.GHI_GIAM_CCDC;
            listRecord.records = [];

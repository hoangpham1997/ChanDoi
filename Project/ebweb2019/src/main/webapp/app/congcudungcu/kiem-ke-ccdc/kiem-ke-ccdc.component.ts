...
import { DDSo_NgoaiTe, DDSo_TienVND, MAU_BO_GHI_SO, SO_LAM_VIEC, TypeID } from 'app/app.constants';
...
@Component({
    selector: 'eb-kiem-ke-ccdc',
    templateUrl: './kiem-ke-ccdc.component.html',
    styleUrls: ['./kiem-ke-ccdc.component.css']
})
export class KiemKeCcdcComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        const searchV = JSON.parse(sessionStorage.getItem(''));
        this.objSearch = searchV ? JSON.parse(sessionStorage.getItem('dataSearchKiemKe')) : {};
    ...
    toggleSearch($event) {
        ...
        this.objSearch.isShowSearch = !this.objSearch.isShowSearch;
    }
    ...
    search() {
                ...
                res => {
                    ...
                    if (res.body.length > 0) {
                        if (JSON.parse(sessionStorage.getItem('itemGOtherVoucher'))) {
                            ...
                            this.selectedRow = this.itiAudits.find(item => item.id === itemID);
                            if (!this.selectedRow) {
                                // this.selectedRow = this.itiAudits[0];
                                this.onSelect(this.itiAudits[0]);
                            } else {
        ...
        sessionStorage.setItem('dataSearchKiemKe', JSON.stringify(this.objSearch));
    }
    ...
    export() {
        event.preventDefault();
        if (!(!this.itiAudits || this.itiAudits.length === 0 || !this.selectedRow)) {
            this.kiemKeCcdcService
                .exportPDFPB({
                    page: this.page - 1,
                    size: this.itemsPerPage,
                ...
                .subscribe(res => {
                    this.refModalService.open(null, EbReportPdfPopupComponent, res, false, TypeID.KIEM_KE_CCDC);
                });
        }
    }
    ...
    doubleClickRow(id: string, check?: boolean) {
        if (id) {
            ...
            this.dataSession.isShowSearch = this.objSearch.isShowSearch;
            ...
            this.router.navigate(['./kiem-ke-ccdc', id, 'edit']);
        }
    }

    deleteAfter() {
        if (this.selectedRows && this.selectedRows.length > 1) {
            this.kiemKeCcdcService.multiDeletePB(this.selectedRows).subscribe(
                res => {
                    if (this.modalRef) {
                        this.modalRef.close();
                    }
            ...
            if (this.selectedRow && this.selectedRow.id) {
                this.kiemKeCcdcService.delete(this.selectedRow.id).subscribe(
                    res => {
                        this.toastr.success(this.translate.instant('ebwebApp.pPDiscountReturn.delete.success'));
                        if (this.modalRef) {
                            this.modalRef.close();
    ...
    registerExport() {
        this.eventSubscriber = this.eventManager.subscribe(`export-excel-${TypeID.KIEM_KE_CCDC}`, () => {
            this.exportExcel();
        });
    }
    ...
    addNew($event) {
        $event.preventDefault();
        // this.router.navigate(['./kiem-ke-ccdc/new']);
    ...
    add(contentDuplicate) {
        ...
        sessionStorage.setItem('objectDateTIAudit', JSON.stringify(objectDate));
        this.router.navigate(['./kiem-ke-ccdc/new']);
        // this.kiemKeCcdcService
        //     .getTIAuditCount({
    ...
    onSelect(item: IGOtherVoucher) {
        ...
        this.index = this.itiAudits.indexOf(this.selectedRow);
        this.rowNum = this.getRowNumberOfRecord(this.page, this.index);
        if (item.id) {
            this.kiemKeCcdcService.findDetailsByID(item.id).subscribe(res => {
                this.itiAuditDetail = res.body.tiAuditDetailByIDDTOS;
                this.tiAuditMemberDetails = res.body.tiAuditMemberDetailDTOS;
                this.viewVouchersSelected = res.body.refVouchers;
            });
        }
    }
    ...
    delete() {
        ...
        if (this.selectedRows && this.selectedRows.length > 1) {
            ...
            // if (
            //     !(
            //         (this.itiAudits && this.itiAudits.length === 0) ||
            //         !this.selectedRow ||
            //         (this.selectedRow.recorded && this.selectedRows.length <= 1) ||
            //         this.checkCloseBook(this.currentAccount, this.selectedRow.postedDate)
            //     )
            ...
            if (this.selectedRow.id) {
                ...
                this.modalRef = this.modalService.open(this.deleteModal, { backdrop: 'static' });
                // this.kiemKeCcdcService.getMaxMonth(this.selectedRow.id).subscribe(res => {
    ...
    continueMultiAction() {
        if (this.typeMultiAction === 1) {
            ...
            listRecord.typeIDMain = TypeID.KIEM_KE_CCDC;
            listRecord.records = [];

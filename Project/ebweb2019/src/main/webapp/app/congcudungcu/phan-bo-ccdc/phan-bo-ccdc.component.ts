...
@Component({
    selector: 'eb-phan-bo-ccdc',
    templateUrl: './phan-bo-ccdc.component.html',
    styleUrls: ['./phan-bo-ccdc.component.css']
})
export class PhanBoCcdcComponent extends BaseComponent implements OnInit, OnDestroy {
                ...
                res => {
                    ...
                    if (res.body.length > 0) {
                        if (JSON.parse(sessionStorage.getItem('itemGOtherVoucher'))) {
                            ...
                            this.selectedRow = this.tiAllocations.find(item => item.id === itemID);
                            if (!this.selectedRow) {
                                // this.selectedRow = this.tiAllocations[0];
                                this.onSelect(this.tiAllocations[0]);
                            } else {
    ...
    export() {
        event.preventDefault();
        if (!(!this.tiAllocations || this.tiAllocations.length === 0 || !this.selectedRow)) {
            this.phanBoCcdcService
                .exportPDFPB({
                    page: this.page - 1,
                    size: this.itemsPerPage,
    ...
    unrecord() {
        ...
        if (this.selectedRows && this.selectedRows.length > 1) {
            ...
            // if (
            //     !(this.tiAllocations && this.tiAllocations.length < 1) ||
    ...
    doubleClickRow(id: string, check?: boolean) {
        if (id) {
            ...
            this.router.navigate(['./phan-bo-ccdc', id, 'edit']);
        }
    }
    ...
    deleteAfter() {
        if (this.selectedRows && this.selectedRows.length > 1) {
            ...
            if (this.selectedRow && this.selectedRow.id) {
                this.phanBoCcdcService.delete(this.selectedRow.id).subscribe(
                    res => {
                        this.toastr.success(this.translate.instant('ebwebApp.pPDiscountReturn.delete.success'));
                        if (this.modalRef) {
                            this.modalRef.close();
    ...
    registerExport() {
        this.eventSubscriber = this.eventManager.subscribe(`export-excel-${TypeID.PHAN_BO_CCDC}`, () => {
            this.exportExcel();
        });
    }
    ...
    add(contentDuplicate) {
        ...
        sessionStorage.setItem('objectDateTIAllocations', JSON.stringify(objectDate));
        this.phanBoCcdcService
            .getTIAllocationCount({
                month: this.selectedMonth,
                year: this.selectedYear
            })
            .subscribe(
                res => {
                    if (res.body > 0) {
                        ...
                        this.router.navigate(['./phan-bo-ccdc/new']);
                    }
                },
                res => {
    ...
    onSelect(item: IGOtherVoucher) {
        ...
        this.index = this.tiAllocations.indexOf(this.selectedRow);
        this.rowNum = this.getRowNumberOfRecord(this.page, this.index);
        if (item.id) {
            this.phanBoCcdcService.findDetailViewPB(this.selectedRow.id).subscribe(res => {
    ...
    delete() {
        ...
        if (
            ...
            if (this.selectedRows && this.selectedRows.length > 1) {
                ...
                // if (
                //     !(
                //         (this.tiAllocations && this.tiAllocations.length === 0) ||
                //         !this.selectedRow ||
                //         (this.selectedRow.recorded && this.selectedRows.length <= 1) ||
                //         this.checkCloseBook(this.currentAccount, this.selectedRow.postedDate)
                //     )
    ...
    continueMultiAction() {
        if (this.typeMultiAction === 1) {
            ...
            listRecord.typeIDMain = TypeID.PHAN_BO_CCDC;
            listRecord.records = [];

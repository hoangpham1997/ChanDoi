...
export class TypeService {
    ...
    getAllTypes(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http.get<IType[]>(this.resourceUrl + '/getAllTypes', { params: options, observe: 'response' });
    }
    ...
    /***
     *
     * @param req example {groupId: 21}
     */
    findAllTypesByGroupId(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http.get<IType[]>(this.resourceUrl + '/find-all-by-type', { params: options, observe: 'response' });
    }

    getTypes(): Observable<EntityArrayResponseType> {
        return this.http.get<IType[]>(this.resourceUrl + '/find-all-types', { observe: 'response' });
    }
}

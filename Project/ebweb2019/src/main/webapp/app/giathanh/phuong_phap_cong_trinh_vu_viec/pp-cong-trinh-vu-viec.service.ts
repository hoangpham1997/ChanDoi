...
export class PpCongTrinhVuViecService {
    ...
    accepted(cPPeriod: ICPPeriod): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(cPPeriod);
        return this.http
            .post<ICPPeriod>(this.resourceUrl + '/accepted', copy, { observe: 'response' })
            .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
    }
}

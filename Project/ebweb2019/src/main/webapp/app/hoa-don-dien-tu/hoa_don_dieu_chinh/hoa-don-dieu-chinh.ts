...
import { DANH_SACH_HOA_DON_DIEU_CHINH, HINH_THUC_HOA_DON_HOA_DON_DIEN_TU } from 'app/app.constants';
...
@Component({
    selector: 'eb-hoa-don-dieu-chinh',
    templateUrl: './hoa-don-dieu-chinh.html',
    styleUrls: ['./hoa-don-dieu-chinh.css']
})
export class HoaDonDieuChinhComponent implements OnInit {
    currentAccount: any;
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/hoa-don-dieu-chinh'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage
    ...
    clickInvoiceNo(id: string, type) {
        const einvoice: IEInvoice = this.eInvoices.find(n => (type === 0 ? n.id : n.iDAdjustInv) === id);
        const einvoiceCopy = Object.assign({}, einvoice);
        if (type !== 0) {
            einvoiceCopy.statusInvoice = 4;
        }
        this.einvocieService
            .viewInvoicePdf({
                id,
                pattern: type === 0 ? einvoiceCopy.invoiceTemplate : einvoiceCopy.invoiceTemplateAdjusted
            })
            .subscribe(
    ...
    searchVoucher(search?) {
        ...
        this.einvocieService
            .searchAll({
                ...
                typeEInvoice: DANH_SACH_HOA_DON_DIEU_CHINH
            })
            .subscribe(

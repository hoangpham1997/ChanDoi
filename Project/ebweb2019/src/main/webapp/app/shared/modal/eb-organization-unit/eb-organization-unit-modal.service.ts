...
export class EbOrganizationUnitModalService {
    ...
    open(data, refModal?, modalData?, isSizeSm?: boolean, typeID?, disableWindowClass?): NgbModalRef {
        ...
        this.isOpen = true;
        const modalRef = this.modalService.open(refModal ? refModal : EbOrganizationUnitModalComponent, {
            size: 'lg',
            backdrop: 'static'
        });
        modalRef.componentInstance.orgs = data;
        if (modalData) {

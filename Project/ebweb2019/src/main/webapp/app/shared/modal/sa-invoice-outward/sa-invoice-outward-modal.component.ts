...
import { SaInvoiceOutwardModalService } from 'app/shared/modal/sa-invoice-outward/sa-invoice-outward-modal.service';

@Component({
    selector: 'eb-pp-order-modal',
    templateUrl: './sa-invoice-outward-modal.component.html',
    styleUrls: ['./sa-invoice-outward-modal.component.css']
})
export class SaInvoiceOutwardModalComponent implements OnInit {
    ...
    constructor(
        ...
        private saInvoiceOutwardModalService: SaInvoiceOutwardModalService,
    ...
    ngOnInit(): void {
        ...
        this.principal.identity().then(account => {
            ...
            if (this.modalData) {
                ...
                this.accountingObjectID = this.modalData.accountingObject;
    ...
    queryData() {
        ...
        this.saInvoiceOutwardModalService
            .find({
                page: this.page - 1,
            ...
            .subscribe(res => {
                this.saInvoice = res.body.content;
                this.saInvoice.forEach(item => {
                    if (this.itemsSelected.some(x => x.sAInvoiceId === item.id)) {
                        item.checked = true;
    ...
    close() {
        ...
        this.eventManager.broadcast({
            name: 'closedSelectedSAInvoiceOutWard'
        });
    }

    apply() {
        this.eventManager.broadcast({
            name: 'selectedSAInvoiceOutWard',
    ...
    isCheckAll() {
        if (this.saInvoice) {
            for (let i = 0; i < this.saInvoice.length; i++) {
                if (!this.saInvoice[i].checked) {
                    return false;
                }
            }
            return true;
    ...
    checkAll() {
        if (this.saInvoice) {
            ...
            for (let i = 0; i < this.saInvoice.length; i++) {
                this.saInvoice[i].checked = !isCheckAll;
                if (!isCheckAll && !this.newList.some(x => x.id === this.saInvoice[i].id)) {
                    this.newList.push(this.saInvoice[i]);
                }
            }
        }
    }

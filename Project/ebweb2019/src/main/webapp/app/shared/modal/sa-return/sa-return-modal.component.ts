...
@Component({
    ...
    templateUrl: './sa-return-modal.component.html',
    styleUrls: ['./sa-return-modal.component.css']
})
export class SaReturnModalComponent implements OnInit {
    ...
    constructor(
        ...
        private ppOrderModalService: SaReturnModalService,
    ...
    queryData() {
        ...
        this.ppOrderModalService
            ...
            .subscribe(res => {
                this.saReturn = res.body.content;
                this.saReturn.forEach(item => {
                    if (this.itemsSelected.some(x => x.saReturnID === item.id)) {
                        item.checked = true;
    ...
    isCheckAll() {
        if (this.saReturn) {
            for (let i = 0; i < this.saReturn.length; i++) {
                if (!this.saReturn[i].checked) {
                    return false;
                }
            }
            return true;
    ...
    checkAll() {
        if (this.saReturn) {
            ...
            for (let i = 0; i < this.saReturn.length; i++) {
                this.saReturn[i].checked = !isCheckAll;
                if (!isCheckAll && !this.newList.some(x => x.id === this.saReturn[i].id)) {
                    this.newList.push(this.saReturn[i]);
                }
            }
        }
    }
